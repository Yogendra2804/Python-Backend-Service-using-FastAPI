...
@router.get("/cards/ORM")
def ORM_get_CARD_details(
    iin: int,
...
@router.post("/cards/ORM")
def ORM_post_get_CARDS(
    data: Value,
    _: None = Depends(get_current_user)
):
    ...
    try:
        ...
        return {
            "message": "ORM expect caught something."
        }


@router.post("/getToken/ORM")
...
def findBINS(
    ...
    try:
        ...
        bin_list = session.scalars(
            select(CARDS.bin_iin).where(
                CARDS.network_Scheme == name
            )
        ).all()
        ...
        if not bin_list:
            ...
            logger.error(
                f"Bin list not found? Where's DB? bin_list = {bin_list}"
            )
